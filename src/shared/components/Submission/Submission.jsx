...
function Submission() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const mailPromise = fetch('http://192.168.1.30/ICAABC/send_mail.php', { method: 'POST', body: formDataToSend, });
                    ...
                    <div className="w-full lg:w-2/3 bg-white shadow-lg rounded-md p-8 border-t-4 border-b-4 border-[#528F62] order-2 lg:order-1">
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            <div>
                                ...
                                <input id="Paper_Title" name="Paper_Title" type="text" placeholder="Enter the title of your paper" value={formData.Paper_Title} onChange={handleFileInputChange} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required />
                            </div>
                            <div>
                                <label htmlFor="Author_FUll_Name" className="block text-lg font-medium text-gray-700">Author(s) Full Name</label>
                                <input id="Author_FUll_Name" name="Author_FUll_Name" type="text" placeholder="Enter the name of the author(s)" value={formData.Author_FUll_Name} onChange={handleFileInputChange} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required />
                            </div>
                            <div>
                                <label htmlFor="Email_Address" className="block text-lg font-medium text-gray-700">Email</label>
                                <input id="Email_Address" name="Email_Address" type="email" placeholder="Enter the author's email" value={formData.Email_Address} onChange={handleFileInputChange} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required />
                            </div>
                            <div>
                                <label htmlFor="Institution_Name" className="block text-lg font-medium text-gray-700">Institution Name</label>
                                <input id="Institution_Name" name="Institution_Name" type="text" placeholder="Enter the author's institution" value={formData.Institution_Name} onChange={handleFileInputChange} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required />
                            </div>
                            <div>
                                <label htmlFor="Paper_Track" className="block text-lg font-medium text-gray-700">Paper Track</label>
                                <select id="Paper_Track" name="Paper_Track" onChange={handleFileInputChange} value={formData.Paper_Track} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required>
                            ...
                            <div>
                                ...
                                <input id="Paper_File" name="Paper_File" type="file" accept=".pdf" onChange={handleFileInputChange} className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-green-600 focus:border-green-600 focus:outline-none" required />
                            </div>
                            <div>
